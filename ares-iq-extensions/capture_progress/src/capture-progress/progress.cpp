...
namespace CaptureProgress {
    ...
    void Progress::_update_rate() {
        std::lock_guard<std::mutex> guard(this->_samples_mtx);
        auto time_diff = std::chrono::duration<double>(std::chrono::system_clock::now() - _start).count();
        if (iszero(time_diff)) {
            return;
        }
        double rate = static_cast<double>(_samples_captured) / static_cast<double>(time_diff) / 1e6;
        std::ostringstream oss;
        oss << std::fixed << std::setprecision(2) << rate << " MS/sec ";
        _rate = oss.str();
        _percent_complete = static_cast<double>(_samples_captured) / static_cast<double>(_total_samples);
    }
    ...
    void Progress::_draw_bar() const {
        uint64_t complete_bars = static_cast<uint64_t>(static_cast<double>(bar_length) * _percent_complete);
        ...
        switch (complete_bars) {
            ...
            case 1:
                ...
                break;
            default:
                complete_bar = std::u32string(complete_bars - 1, bar_char) + progress_stub;
        ...
        std::cout << current_progress_color << utf8_complete_bar << remaining_progress_color << utf8_remaining_bar
                  << " " << magenta_color << static_cast<int>(_percent_complete * 100.0) << "% ";
    }

    void Progress::_draw_rate() {
        std::cout << magenta_color << _rate;
    }
    ...
    void Progress::_draw_time_elapsed() {
        ...
        std::ostringstream oss;
        oss << std::setw(1) << std::setfill('0') << hrs << ":" << std::setw(2) << std::setfill('0') << min << ":"
