...
import platform
import pathlib
from .sm_path import lib_path


if platform.system() != "Linux":
    raise RuntimeError("OS not supported")

smlib = CDLL(f"{pathlib.Path(__file__).parent.resolve()}/{lib_path()}/libsm_api.so")


class SMDeviceError(Exception):
    def __init__(self, code):
        super().__init__()
        self._msg: bytes = sm_get_error_string(code)['error_string']
        self._code = code
